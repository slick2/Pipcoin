PipCoin-qt: Qt4 GUI for PipCoin
===============================
...
Alternatively, install Qt Creator and open the `PipCoin-qt.pro` file.

An executable named `PipCoin-qt` will be built.
...
A warning for people using the *static binary* version of PipCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of PipCoin is linked against libdb5.3.
...
installed causes PipCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
