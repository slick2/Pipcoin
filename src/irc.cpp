...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #PipCoinTEST2\r");
            Send(hSocket, "WHO #PipCoinTEST2\r");
        } else {
            // randomly join #PipCoin00-#PipCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #PipCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #PipCoin%02d\r", channel_number).c_str());
        }
